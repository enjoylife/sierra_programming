...
/*********************************/
  ...
  //THIS IS FOR EDITING AN EXISTING ENTRY 
    if(exisiting){
             fp = fopen("sportstorerec.db", "r+b");
            if (fp==NULL) {fputs ("\n***File error****\n\n",stderr); return;}
                report();
            choice=validInt("Choose a Record to Change from the left Most column\n", 0,INT_MAX);
                spot = (long)(choice* sizeof(record));
            ...
            show(record.product_number, record.product_type, record.product_quantity,
                    record.product_cost, record.product_price, record.description);
             pick= toupper(getValidChar("Are you sure? (type Y/N)\n"));
                getchar();
               if (pick=='Y'){
                fseek(fp,spot, SEEK_SET);
               ...
               printf("Record Not Changed");
           }
          do{
           pick= toupper(getValidChar("Would you like to change another one again? (type Y/N)\n"));
           getchar();
        }while(!(pick=='Y'||pick=='N'));
        //Cant forget this !
        fclose(fp);
...
void deleteRec(){
    ...
    report();
    choice=validInt("Choose a Record to delete from the left Most column\n", 0,INT_MAX);
    spot = (long)(choice* sizeof(record));
    fseek(fp,spot, SEEK_SET);
    fread(&record, sizeof(record), 1, fp);
    printf("Are you sure you want to delete %d for:\n",choice);
    show(record.product_number, record.product_type, record.product_quantity,
            record.product_cost, record.product_price, record.description);
   do{
   pick= toupper(getValidChar("(type Y/N)\n"));
   getchar();
    }while(!(pick=='Y'||pick=='N'));
   if (pick=='Y'){
  ...
  do{
   pick= toupper(getValidChar("Would you like to Delete Again? (type Y/N)\n"));
   getchar();
    }while(pick=='Y');
    //Cant forget this !
    fclose(fp);
...
void changeRec(){
    //Changed the function to accept a bool to decide if it is already in the database
       addEntry(true);
   }
...
void report()
{
    ...
    printf("%s%7s%7s%15s%5s%7s%9s%10s\n","Rec #", "prod#", "Type", "Description", "Qty", "Cost", 
            "Price", "Profit");
    printf("----------------------------------------------------------\n");
    ...
    printf("%d %7d %7d %15s %5d %7.2lf %9.2lf %10.2lf\n",recCounter,record.product_number, 
